"""쿠팡 파트너스 URL → 인스타그램 카드뉴스 생성 (명령줄).

사용 예시:
    python -m src.coupang "https://link.coupang.com/a/xxxxx"
    python -m src.coupang "https://link.coupang.com/a/xxxxx" --cards 3 --theme "웜 크림"
    python -m src.coupang "https://..." --name "상품명" --price "29,900원"   # 정보 직접 지정
...
from . import card_generator, coupang_fetcher
...
def run(
    ...
    if not product.get("name"):
        raise RuntimeError(
            "상품 정보를 찾지 못했습니다. 쿠팡이 접근을 차단했을 수 있어요. "
...
def main() -> None:
    ...
    parser.add_argument("url", help="쿠팡 파트너스 URL (예: https://link.coupang.com/a/xxxxx)")
    ...
    try:
        run(args.url, cards=args.cards, theme=args.theme, ratio=args.ratio,
